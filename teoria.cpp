...
// Умные указатели и динамичесский массив

shared_ptr<int[]> ptr (new int [SIZE] {312,321,4,534,2});
// создали умный указатель на динамичесский массив
const int size = 5;
shared_ptr<int[][size]> ptr(new int[size][size]);
// создали умный указатель на двухмерный динамичесский массив

// Auto 

auto x = 42;
// Тип переменной x будет автоматически определен как int
auto name = "John"; 
// Тип переменной name будет автоматически определен как const char*
auto result = calculateResult();
// Тип переменной result будет автоматически определен на основе возвращаемого значения функции calculateResult()

// Использование auto с итератором
for (auto it = numbers.begin(); it != numbers.end(); ++it) {
    // Тип переменной it будет автоматически определен как vector<int>::iterator
    cout << *it << endl;
}
...
for (const auto& index : numbers) {
    // создаем цикл for который будет пробегаться по всем обькетам вектора 
    // Тип переменной number будет автоматически определен как const int
    cout << index << " ";
    // выводим то что получили из вектора
}

// typedef

typedef int myInt;
// Создали новый тип myInt который будет означать команду int
myInt number = 42;
// Создали переменную number которая будет иметь тип myInt(тобиш будет int)

// Pair

// pair предоставляет удобный способ объединить два значения разных типов в одну структуру данных. 
pair<int, string> myPair(42, "String");
// Создали пару с ключом типа int и значением типa string
myPair.first;
// Получили ключ
myPair.second;
// Получили значение
make_pair(T1&& first, T2&& second);
// Создали пару с ключом типа T1 и значением T2
...
myVector.erase(индекс);
// удаляет элемент по указаному индексу
myVector.erase(индекс, второй индекс);
// удаляет числа в диапозоне от первого до второго индексов
myVector.empty();
...
myVector.begin()
// возвращает итератор, указывающий на первый элемент вектора myVector.
myVector.end()
// возвращает итератор, указывающий на элемент, следующий за последним элементом вектора myVector

// Итератор 

vector<int> myVector = {1,9, 44, 422, 676, 78};
// создали вектор чисел
vector<int>::iterator it;
// итератор для вектора myVector должен иметь такой же тип данных как и myVector(тоесть итератор должен быть типа vector<int>)
it = myVector.begin();
// присваивает итератору it значение, указывающее на первый элемент вектора myVector
it = myVector.end()
// присваивает итератору it значение, указывающее на позицию, следующую за последним элементом вектора myVector.
vector<int>::const_iterator it;
// создает константный итератор it
vector<int>::const_iterator i = myVector.cbegin()
// присваивает константному итератору i константный итератор, указывающий на первый элемент вектора myVector.
vector<int>::const_iterator i = myVector.cend()
// присваивает константному итератору i константный итератор, указывающий на позицию, следующую за последним элементом вектора myVector
vector<int>::reverse_iterator i = myVector.rbegin()
// присваивает обратному итератору i значение, указывающее на последний элемент вектора myVector.
vector<int>::reverse_iterator i = myVector.rend()
// присваивает обратному итератору i значение, указывающее на позицию перед первым элементом вектора myVector.
advance(it, 2)
// перемещает итератор it вперед на 2 позиции
myVector.insert(it, 999)
// вставляет значение 999 в позицию, указанную итератором it, вектора myVector
myVector.erase(itErase, itErase + 3)
// удаляет элементы вектора myVector в диапазоне от itErase до itErase + 3.

// List https://www.youtube.com/watch?v=tYkaWCkAVJI&list=PLQOaTSbfxUtCrKs0nicOg2npJQYSPGO9r&index=158

#include <list>
// библиотека для листа
...
// Создание итератора, указывающего на начало списка
myList.push_back(5);
// Добавление элемента 5 в конец списка
myList.push_front(432);
// Добавление элемента 432 в начало списка
myList.size()
// Получение текущего размера списка (количество элементов)
myList.sort();
...
myList.unique();
// Удаление всех дубликатов из списка(которые идут друг за другом!!!)
myList.reverse();
// Разворот списка (перестановка элементов в обратном порядке)
myList.insert(it, 3)
// Вставка значения 3 перед элементом, на который указывает итератор it
myList.erase(it);
// Удаление элемента, на который указывает итератор it
myList.remove(99);
// Удаление всех элементов со значением 99 из списка
myList.assign(5,99);
// Замена содержимого списка на 5 элементов со значением 99
myList.assign(myList2.begin(), myList2.end());
// Замена содержимого списка на элементы из другого списка myList2
myList.clear()
// Удаление всех элементов из списка, делая его пустым

// Forward List(Односвязный список)

#include <forward_list>
// подключаем библиотеку для односвязного списка
forward_list<int> fl {32,42};
// Инициализация односвязного списка с исходными цифрами(32 и 42)
fl.push_front(1);
// Вставка элемента в начало списка
forward_list<int>::iterator it = fl.begin();
// Получение итератора на первый элемент списка
fl.insert_after(it, 999);
// Вставка элемента после указанного итератора
it = fl.before_begin();
// Переустановка итератора на позицию перед началом списка(Что бы можно было изменить цифру в начале списка)
...
arr.fill(-1);
// Заполняем все элементы массива значением -1
arr.at(2);
// Получаем элемент массива с индексом 2(но если индекс будет выходить за размеры массива то будет бросаться ошибка)
arr.size()
...
// Deque https://youtu.be/bwW4Dk-CoxA

// deque - это контейнер в стандартной библиотеке C++, который представляет собой двустороннюю очередь. 
// Он обеспечивает быстрый доступ и вставку элементов как в начало, так и в конец контейнера.
// Что-то среднее между вектором и двухсвязным списком. Данные состоят из массива и еще имеют указатель на след. и преведущий массив.

// SET | MULTISET

// set: 
    // Бинарный список
    // Хранит уникальные значения, исключая дубликаты. 
    // Элементы автоматически упорядочиваются по возрастанию с помощью оператора  < или с помощью пользовательского компаратора. 
    // Вставка, удаление и поиск элементов выполняются за время O(log n), где n - количество элементов в контейнере. 

set<int> mySet;
// обьявление бинарного списка
mySet.insert(1);
...
// Метод find() возвращает итератор на найденный элемент, если такой элемент существует, или итератор, указывающий на конец контейнера, если элемент не найден.
...
// multiset: 
    // бинарный список
    // Позволяет хранить дубликаты элементов, то есть может содержать несколько экземпляров одного и того же значения. 
    // Элементы автоматически упорядочиваются по возрастанию с помощью оператора  < или с помощью пользовательского компаратора. 
    // Вставка, удаление и поиск элементов выполняются за время O(log n), где n - количество элементов в контейнере. 
multiset<int> myMultiset {432,543,2,2,2,2,5,6};
// Обьялвение мультисетного бинарного списка
myMultiset.lower_bound(1)
// Возвращает итератор на первый элемент, который больше или равен заданному значению.
myMultiset.upper_bound(1);
// возвращает итератор на первый элемент, который больше указанного значения, или итератор, указывающий на конец контейнера, если все элементы меньше или равны указанному значению.
myMultiset.equal_range(1);
// метод позволяет найти все элементы в множестве, которые равны заданному значению.

// MAP | MULTIMAP

// Основное отличие между map и multimap заключается в том, что map хранит уникальные ключи, тогда как multimap позволяет хранить неуникальные ключи.
#include<map>
// подключение библиотеки Мар 
map<int, string> myMap;
// Создали бинарное дерево пар у которых ключ будет типа int а значение этого ключа будет типа string
myMap.emplace(ключ, значение);
// Создание пары 
myMap.insert(make_pair(Ключ, значение));
// Аналогичное создание пары но используя функцию insert и make_pair
cout << myMap[3] << endl;
// Вывод значения которое хранится в третьем ключе
for(auto &item: myMap){
    cout << item.first << ":" << item.second << endl;
}
// Цикл который перебирает все элементы в Мар

// Stack

#include<stack>
// библиотека для стэка
st.emplace(8);
// добавление элемента в стэк(более новый и быстрый метод, лучше использовать его)
st.push(2);
// добавление элемента в стэк
st.pop();
// удаление элемента из стэка
st.top();
// возвращает значение вершины стэка(последнего элемента)
st.empty();
...
// цикл который выводит все элементы в стэке и потом их удаляет
stack<int, list<int>>st2;
// Этот код объявляет стек st2 с типом элементов int и внутренним контейнером list<int>. 
// В данном случае, внутренний контейнер стека будет list<int>, что позволяет использовать list для хранения элементов в стеке.
...
// Этот набор инструментов позволяет разрабатывать параллельные и конкурентные программы, которые могут выполнять несколько потоков одновременно
// Каждый поток занимает ядро процессора
...
thread название();
// создали поток (он являеться обьектом класса)
this_thread::
// обращение к текущему потоку
this_thread::get_id()
// получаем айди текущего потока
this_thread::sleep_for(chrono::seconds(1));
// this_thread:: - обращение к текущему потоку
// sleep_for() - команда которая обозначает паузу 
// chrono::seconds(1) - говорим ждать одну секунду
thread th(DoWork,"Hello");
// передаем в поток функцию DoWork и параметр функции Hello  (тоесть передали в поток DoWork("Hello"))
название_потока.join();
// используется для дожидания завершения выполнения потока
название_потока.detach();
// после вызова родительский поток и поток-потомок будут работать независимо друг от друга, если родительский поток закончится, 
// а потомок будет работать то потомок оборвет сови действия и закончится
...
// Оператор который заменяет два вложеных оператора
for (int i = 0, f = 0; i < SIZE && f < SIZE_1; i++, f++) {
    // Тело цикла
}
